...
function compareVersions (version1, version2) {
  ...
  if (version1.length === 1 || version2.length === 1) {
    return parseFloat(version1[0]) >= parseFloat(version2[0]);
  }

  else {
    let counter = 0;
    while (counter < version1.length && counter < version2.length) {
      if (version1[counter] === version2[counter]) {
        counter++;
        if (counter === version1.length && counter != version2.length) {
          return false;
        }
        else if (counter === version2.length && counter != version1.length) {
          return true;
        }
      }
      else {
        return parseFloat(version1[counter]) >= parseFloat(version2[counter]);
      }
    }
  }
}
