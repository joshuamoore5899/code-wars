...
function tug_o_war(teams) {
  let team1 = teams[0].reduce((sum, num) => sum + num, 0);
  let team2 = teams[1].reduce((sum, num) => sum + num, 0);
  if (team1 > team2) {
    return 'Team 1 wins!';
  }
  else if (team2 > team1) {
    ...
    return teams[0][0] > teams[1][teams[1].length - 1] ?
    'Team 1 wins!' : teams[1][teams[1].length - 1] > teams[0][0] ?
    'Team 2 wins!' : "It's a tie!";
  }
}
