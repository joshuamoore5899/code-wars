...
function rot(strng) {
    // your code
  strng = strng.split('').reverse().join('');
  return strng;
}

function selfieAndRot(strng) {
    // your code
  let first = strng.split('\n');
  let temp = "";
  for (let i = 0; i < first[0].length; i++) {
    temp += '.';
  }
  let second = oper(rot, strng);
  second = second.split('\n');
  second = second.join('\n' + temp);
  first = first.join(temp + '\n');
  let ans = first.concat(temp).concat('\n').concat(temp).concat(second);
  return ans;
}
