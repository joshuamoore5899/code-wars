...
function countVegetables(string){
  let vegetables = ["cabbage", "carrot", "celery", "cucumber", "mushroom", "onion", "pepper", "potato", "tofu", "turnip"]
  let list = string.split(" ");
  let tracker = {};
  for (let i = 0; i < list.length; i++) {
    if (vegetables.indexOf(list[i]) != -1) {
      tracker[list[i]] = tracker[list[i]] + 1 || 1;
    }

  }
  let final = [];
  for (const key in tracker) {
    final.push([tracker[key], key])
  }
